...
- [ ] **Allocation:** Missing upline levels are reallocated to marketing pool (`marketing_pool_ledger` with `source_type: 'missing_upline_reallocation'`). Consider moving allocation into a single DB transaction (e.g. RPC) for stronger consistency.
