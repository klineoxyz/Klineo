...
adminRouter.get('/user-discounts', async (req, res) => {
  ...
  try {
    const { data: discounts, error } = await client
      .from('user_discounts')
      .select('id, user_id, code, scope, onboarding_discount_percent, onboarding_discount_fixed_usd, onboarding_claimed_at, trading_discount_percent, trading_package_ids, trading_max_packages, trading_used_count, status, created_by, created_at, updated_at')
      .order('created_at', { ascending: false });
    ...
    const formatted = discounts?.map((d: any) => ({
      ...
      onboardingClaimedAt: d.onboarding_claimed_at || null,
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    if (kind === 'joining_fee') {
      ...
      if (couponCodeRaw?.trim()) {
        const { data: udOnboard } = await client
          .from('user_discounts')
          .select('id')
          ...
          .maybeSingle();
        if (udOnboard) {
          await client.from('user_discounts').update({ onboarding_claimed_at: now, updated_at: now }).eq('id', (udOnboard as { id: string }).id);
        }
      }
