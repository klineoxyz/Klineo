...
export function MacdChart({ data, height = 120 }: { data: OhlcvRow[]; height?: number }) {
  ...
  const macdSeriesData = macd.map((d) => ({ time: toUtc(d.time), value: d.value }));
  const signalSeriesData = signal.map((d) => ({ time: toUtc(d.time), value: d.value }));
  const histSeriesData = histogram.map((d) => ({ time: toUtc(d.time), value: d.value, color: d.color }));
