-- KLINEO: Atomic allocation of purchase revenue (70/20/10, 7-level referral).
-- Single transaction: no partial credits. Idempotent via purchase_allocation_runs.
-- Call from backend: supabase.rpc('allocate_purchase_revenue', { p_purchase_id: '<uuid>' }).
...
DECLARE
  ...
  v_referrer_id UUID;
  v_level INT;
  -- Level weights (L1..L7): 30, 20, 10, 8, 6, 4, 2; sum = 80. Pool = 70%.
  v_level_pct NUMERIC[] := ARRAY[
    30.0/80*70, 20.0/80*70, 10.0/80*70, 8.0/80*70, 6.0/80*70, 4.0/80*70, 2.0/80*70
  ];
  v_level_amt NUMERIC[];
...
BEGIN
  ...
  -- 3) Compute amounts (round per-share to 2 decimals; remainder -> marketing)
  ...
  v_marketing_base := ROUND((v_amount * 10 / 100)::numeric, 2);
  ...
  -- 5) Direct 10% to marketing_pool_ledger
  ...
  INSERT INTO public.purchase_revenue_splits (purchase_id, split_type, amount, currency, source_detail)
  VALUES (p_purchase_id, 'marketing', v_marketing_total, v_currency, 'direct_10pct_plus_reallocations_and_remainder');
...
COMMENT ON FUNCTION public.allocate_purchase_revenue(UUID) IS
  'Atomically allocates purchase revenue: 7-level referral earnings, marketing pool, revenue splits. Idempotent. Call with service_role.';
