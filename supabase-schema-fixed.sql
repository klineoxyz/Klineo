-- KLINEO Supabase Database Schema (FIXED)
-- Run this in Supabase SQL Editor after creating project
-- FIXED: Removed OLD/NEW from RLS policy (not supported in RLS)
...
-- Users can update email, but role changes require backend with service_role key
...
-- Step 1: Create user in Supabase Auth (Authentication → Users → Add User)
-- Step 2: Then run this UPDATE query separately
